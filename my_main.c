...
void first_pass() {
  ...
  if (varyfound == 1 && framefound == 0){
    printf("Error: VARY is present but FRAMES is not\n");
    exit(0);
  }
  if (framefound == 1 && basefound == 0){
...
struct vary_node ** second_pass() {
  ...
  for (i = 0; i < lastop; i++){
    if (op[i].opcode == VARY){
      ...
      double steps = (op[i].op.vary.end_val - op[i].op.vary.start_val) / (end - start);
      // printf("steps %0.2lf\n", steps);
      // printf("start %d to end %d\n", start, end);
      int j;
      for (j = start; j < end; j++){
        curr = malloc(sizeof(struct vary_node));
        strcpy(curr->name, op[i].op.vary.p->name);
        curr->value = op[i].op.vary.start_val + steps * (j - start);
        curr->next = knobs[j];
        // printf("%0.2lf\n", curr->value);
        knobs[j] = curr;
      }
...
void my_main() {
  ...
  if (num_frames > 1){
    ...
    for (f = 0; f < num_frames; f++){
      ...
      // printf("ahhhh\n");
      while (vn){
        SYMTAB *symbol = lookup_symbol(vn->name);
